...
\section{Boolsche Algebra}
	...
	\begin{tabular}{l|l|l}
		...
		De Morgan   & $\overline{x \cdot y} = \overline{x} + \overline{y}$  & $\overline{A \cap B} = \overline{A} \cup \overline{B}$\\
		            & $\overline{x + y} = \overline{x} \cdot \overline{y}$ 	& $\overline{A \cup B} = \overline{A} \cap \overline{B}$\\ \hline
		\textbf{Resolution}	&Produktterm											&Summenterm												\\
		Speziell	& $x\cdot a + \overline x \cdot a = a$ 					&$\left(x+a\right)\cdot\left(\overline{x}+a\right) = a$	\\
		Allgemein	& \multicolumn{2}{l}{$x\cdot a+\overline{x}\cdot b = x \cdot a + \overline{x} \cdot b + a \cdot b$} 			\\
					& \multicolumn{2}{r}{$\left(x + a\right)\cdot \left(\overline{x}+b\right)=\left(x + a\right)\cdot \left(\overline{x}+b\right)\cdot\left(a+b\right)$}	\\
	\end{tabular}
	...
	\begin{tabular}{l|l|l}	
		...
		KDNF (CSOP)& Summe aller Minterme $\sum m_i$& WT: 1-Zeilen sind Minterme\\
		KKNF (CPOS)& Produkt aller Maxterme $\prod M_i$& WT: 0-Zeilen negiert sind Maxterme \\
	...
	\end{tabular}
	\\ \\
	\textbf{FPGA}: \textit{F}ield \textit{P}rogrammable \textit{G}ate \textit{A}rray\\
	\textbf{LUT}: \textit{L}ook \textit{U}p \textit{T}able\\ 
